...
export default function Trainers() {
  ...
  useEffect(() => {
    sanityClient
      ...
      .then((data) => setTrainers(data.map(t => ({
        ...t,
        specialties: (t.specialties ?? []).map(s => s.toLowerCase().replace(/\s+/g, '-')),
      }))))
  ...
  const availableFilters = [
    { value: 'all', label: 'All' },
    ...Array.from(new Set(trainers.flatMap(t => t.specialties ?? [])))
      .sort()
      .map(value => ({ value, label: FILTER_LABELS[value] ?? value })),
  ]
...
function TrainerCard({ trainer }: TrainerCardProps) {
  ...
  return (
    <article
      className="rounded-2xl overflow-hidden transition-all duration-200 hover:-translate-y-1 group cursor-pointer"
      style={{
